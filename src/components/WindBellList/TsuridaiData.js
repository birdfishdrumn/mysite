export const TsuridaiData = [
  {
    title: "鉄棒型つり台",
    description:
      "スタンダードな鉄棒型のつり台です。風鈴を二つ飾れる程幅の広いのが特徴です。",
    image: "tsurindai_tetsubou.jpg",
  },
  {
    title: "桐のつり台",
    description:
      "桐の木を使ったつり台で、どこから見ても楽しむことが出来る立体的な構造になっています。お気に入りの一つを飾るのに特にお勧めです。",
    image: "tsuridai_meddle.jpg",
  },
  {
    title: "行灯型つり台",
    description:
      "お昼は風鈴を飾る釣り台として、夜は行灯のように照明としてもお使いいただけます。暑い夏の夜をうちわで仰ぎながら行灯照明に灯りを灯すと風情を感じられて良いですよ。",
    image: "tsuridai_andon.jpg",
  },
]

export const WorkshopKitFlow = [
  {
    title: "風鈴に絵を描く",
    description: "風鈴の内側から絵を描きます。",
    image: "paint_workshopkit.jpg",
  },
  {
    title: "短冊をつける",
    description: "短冊をつけて風鈴の音がなるようにしましょう。",
    image: "tanzaku_ito2.jpg",
  },
  {
    title: "風鈴を吊り下げる",
    description: "実際に吊り下げて音を楽しみましょう。",
    image: "do_hang.jpg",
  },
]
