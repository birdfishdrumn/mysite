...
const Earring = () => {
  const data = useStaticQuery(graphql`
    query EarringQuery {
      allProducts(filter: { category: { eq: "イヤリング" } }) {
        edges {
          node {
            name
            id
  ...
  return (
    <div>
      <section className="center">
        <SubTitle>
          <Trans>イヤリング・ピアス</Trans>
        </SubTitle>
        <Description left width="half">
          <Trans>
            当店では風鈴の技術を応用して風鈴型のイヤリング、ピアスの製造も行っております。とても小さい商品でも、絵柄は内側から手描きで彩色してます。浴衣などと非常に相性が良いので夏のファッションとしていかがでしょうか。
          </Trans>
        </Description>
        <FlexNav>
